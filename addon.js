...
const LISTS_CONFIG_FILE = path.join(__dirname, 'config', 'lists.json');
...
function loadConfig() {
  try {
    if (!fs.existsSync(LISTS_CONFIG_FILE)) {
      console.error('❌ lists.json file not found');
      return { lists: [] };
    }
    
    const data = fs.readFileSync(LISTS_CONFIG_FILE, 'utf8');
